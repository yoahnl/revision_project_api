...
V1-017 est implémenté côté backend API pour les deux nouveaux types rich closed V1-B: `timeline` et `date_slider`. Le contrat, la validation, la génération Genkit mockable, le mapping public anti-fuite, le parsing des réponses submit, le scoring post-submit, les corrections, les fixtures V1-B et les smoke/e2e ciblés ont été ajoutés sans modifier Prisma et sans lancer de provider IA réel.

Les six types V1-A restent couverts et le mix par défaut à 6 questions reste V1-A. Les types V1-B sont inclus automatiquement à partir d’un volume compatible, ou via `questionTypeMix` explicite.
...
- Tests rich closed, Genkit et activity module
...
- `prisma/demo-seed.ts` inspecté: aucune modification nécessaire
- `docs/v1/ROADMAP_EXECUTION_PLAN_V1.md`
- Rapports V1-015/V1-016 consultés pour cohérence documentaire
...
Aucun autre repo n’a été modifié. Aucun commit, push, tag, merge, rebase, amend, reset ou migration Prisma n’a été lancé.

## 4. Périmètre réalisé

Backend API:

- Ajout du contrat interne/public/answer/correction pour `timeline`.
- Ajout du contrat interne/public/answer/correction pour `date_slider`.
- Validation stricte des deux types.
- Mapping public sans correction pré-submit.
- Parsing submit strict dans le controller et le scorer.
- Scoring backend post-submit.
- Fixtures V1-B séparées en 8 questions pour préserver les fixtures V1-A à 6 questions.
- Smoke e2e start/get/submit/result V1-B avec anti-fuite et invalides ciblés.
- Genkit schema/prompt/mix mis à jour sans provider réel.
- Documentation V1 mise à jour.

Frontend app:

- Réalisé dans le repo app, rapport séparé: `revision_app/docs/v1/ROADMAP_EXECUTION_LOT_V1_017_TIMELINE_DATE_SLIDER.md`.
...
- `events: Array<{ id, label, description? }>`
- `correctOrder: string[]`
- `explanation: string`

`timeline` public pré-submit:

- expose `id`, `questionKind`, `prompt`, `instruction?`, `difficulty`, `cognitiveSkill`, `sourceChunkIds`, `events`.
- n’expose pas `correctOrder`, `explanation`, `correction`, `score`, `feedback` ni `correct*`.
...
- `{ isCorrect, submittedAnswer, correctAnswer: { correctOrder }, explanation }`

`date_slider` interne:

- `questionKind: 'date_slider'`
- `minYear`, `maxYear`, `step`
- `correctYear`
- `toleranceYears`
- `explanation`
...
- expose `id`, `questionKind`, `prompt`, `instruction?`, `difficulty`, `cognitiveSkill`, `sourceChunkIds`, `minYear`, `maxYear`, `step`, `toleranceYears`.
- n’expose pas `correctYear`, `explanation`, `correction`, `score`, `feedback` ni `correct*`.

`date_slider` answer:

- `{ questionId, questionKind: 'date_slider', year: number }`

`date_slider` correction:

- `{ isCorrect, submittedAnswer, correctAnswer: { correctYear, minAcceptedYear, maxAcceptedYear }, explanation }`

## 6. Genkit

- Le schema Zod accepte `timeline` et `date_slider` valides.
- Le schema refuse les champs libres inattendus via schemas stricts.
- Le prompt principal mentionne explicitement les deux nouveaux types comme fermés V1-B.
- Le prompt interdit les réponses libres, HTML, SVG, Mermaid et widget libre.
- Le prompt interdit les futurs types hors périmètre, dont `true_false_grid`, `cause_consequence`, `institution_matrix`, `diagram_labeling`, `image_choice`.
- Le prompt de réparation rappelle les contraintes V1-B.
- Le mix par défaut `questionCount = 6` reste V1-A: les six types existants à 1 chacun, V1-B à 0.
- Pour `questionCount >= 8`, le profil peut inclure `timeline` et `date_slider`.
- Les tests Genkit utilisent des réponses mockées; aucun provider réel n’est lancé.
...
- Libellés non vides.
- `correctOrder.length === events.length`.
- `correctOrder` contient exactement les IDs des events.
- Réponse submit complète, sans doublon et sans ID inconnu.
- Scoring V1: ordre exact uniquement; pas de partial score.

Date slider:

- `minYear < maxYear`.
- `step >= 1`.
- `correctYear` entier dans les bornes.
- `toleranceYears >= 0`.
- Réponse submit entière, dans les bornes et compatible avec le step.
- Scoring: correct si `abs(year - correctYear) <= toleranceYears`.

Erreurs rejetées:

- Timeline avec moins de 3 events.
- Timeline avec IDs dupliqués, ordre incomplet ou ID inconnu.
- Submit timeline avec doublon, ID inconnu ou ordre incomplet.
- Date slider avec bornes invalides, step invalide, correctYear hors bornes.
- Submit date slider hors bornes ou non entier.

## 8. Flutter

Le frontend a été modifié dans `revision_app` et documenté dans son propre rapport. Côté API, le contrat public et les corrections post-submit fournissent les données nécessaires aux modèles, parser, widgets minimaux et correction UI sans calcul de score côté Flutter.

## 9. Anti-fuite

Champs privés retirés pré-submit:
...
Les tests couvrent le mapping public start/get, les scans anti-fuite récursifs et les réponses post-submit. Les corrections et explications restent autorisées uniquement après submit/result.
...
- Validator rich closed: fixtures V1-B valides et invalides timeline/date_slider.
- Public mapper: retrait des champs privés V1-B.
- Scorer: réponses correctes/incorrectes, erreurs submit et corrections V1-B.
- Generation profile: mix V1-A conservé à 6, V1-B inclus à 8+.
- Genkit generator: schema V1-B, prompt, absence de widget libre, refus de champs libres.
- E2E critical paths: start/get/submit/result V1-B, anti-fuite, invalides timeline/date_slider.
- Activity module spec: exhaustivité des mappings publics V1-B.

## 12. Validations lancées avec résultats

Depuis `api`:

- `npm test -- rich-closed --runInBand`: passé, 8 suites, 109 tests.
- `npm test -- activities --runInBand`: passé, 17 suites passées, 1 skipped; 206 tests passés, 1 skipped.
- `npm run test:e2e -- --runInBand`: passé, 2 suites, 20 tests.
...
- Aucune migration Prisma: non nécessaire, le contrat rich closed est applicatif/JSON.
- Aucune action Dokploy: hors périmètre.
- Aucun seed destructif ni reset DB: hors périmètre et interdit.

## 14. Risques restants

- Le scoring timeline est volontairement binaire; un partial score pourrait être utile plus tard mais serait un choix de produit séparé.
- La compatibilité `step` côté date slider suppose que la valeur utilisateur suit `minYear + n * step`.
- Les prompts Genkit élargissent l’allowlist mais gardent V1-B hors mix par défaut à 6 questions pour réduire le risque de régression V1-A.
...
- Backend contract: `timeline` et `date_slider` ajoutés sans créer de types V1-018.
- Backend Genkit: schemas stricts, prompt V1-B, widget libre interdit, provider réel non lancé.
- Backend scoring: scoring serveur uniquement, binaire timeline, tolérance date_slider.
- Public mapper anti-fuite: aucun `correctOrder`, `correctYear`, `explanation`, `correction`, `score`, `feedback` pré-submit.
- Flutter parser/model: traité dans le rapport app.
- Flutter widgets: traité dans le rapport app.
- Correction UI: traité dans le rapport app.
- Tests: suites ciblées et e2e passées.
- Sécurité: pas de secret, pas de token, pas d’URL privée, pas de provider IA réel.
...
- Pas de V1-018.
- Pas de score Flutter côté API; le score reste calculé backend.
- V1-A toujours couvert.
- Tests V1-B verts.

## 17. Critique honnête du prompt initial

Le prompt est cohérent et suffisamment borné. La seule tension réelle est l’exigence de recopier le contenu complet de tous les fichiers créés/modifiés dans le rapport: pour le rapport lui-même, cela crée une récursion infinie. La résolution documentée est de considérer que le présent document est son propre contenu complet et de recopier tous les autres fichiers touchés.
