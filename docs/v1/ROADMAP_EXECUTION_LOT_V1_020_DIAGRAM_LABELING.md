...
V1-020 est implémenté côté API et validable : le type fermé `diagram_labeling` est supporté dans le contrat rich closed, la validation, Genkit mocké, le mapper public pré-submit, le parsing submit, le scoring/correction post-submit, les fixtures et le smoke e2e.

Aucune migration Prisma, aucun provider IA réel, aucun widget libre, aucun rendu HTML/SVG/Mermaid/Markdown/Canvas/image URL n'a été ajouté. V1-021 n'a pas été traité.

## 2. Sources inspectées

- `src/modules/activities/application/rich-closed-questions/rich-closed-question.types.ts`
- Validators rich closed, quality gates, public mapper, scorer, submit/get/start/result.
- Fixtures rich closed V1-A, V1-B, V1-C et tests associés.
- Générateur Genkit rich closed, génération profile et tests mockés.
...
- `prisma/demo-seed.ts` inspecté pendant l'audit initial, non modifié.
...
- Derniers commits initiaux : `4f51fcd 019: Intégration de la matrice institutionnelle`, `4092741 018: Intégration de la grille vrai/faux avec cause et conséquence`, `8e8cb5a 017: Intégration du slider de date pour la timeline et corrections associées`.
- Status final non commité : fichiers modifiés/créés listés en section 10.
- Repo frontend également modifié, avec rapport séparé dans `revision_app/docs/v1`.
...
- Ajout de `diagram_labeling` à la union `RichClosedQuestionKind`.
- Contrat interne/public typé pour diagram, nodes, groups, edges, slots et valeurs.
- Validation stricte des bornes nodes/edges/groups/slots/options et des références.
- Scan anti-rendu arbitraire sur les clés interdites : `html`, `svg`, `rawSvg`, `mermaid`, `markdown`, `widget`, `component`, `renderPayload`, `style`, `css`, `script`, `imageUrl`, `assetUrl`, `canvas`, `code`, `markup`.
- Mapper public pré-submit sans `correctValues`, `explanation`, correction, score ou feedback.
- Parsing DTO submit et rejet des champs libres ou de rendu arbitraire.
- Scoring full-correct backend uniquement.
- Correction post-submit avec valeurs soumises/correctes.
- Fixture V1-C full à 12 questions.
- Smoke e2e V1-020.

### Frontend app

Réalisé dans le repo `revision_app`; voir rapport app séparé.

## 5. Contrat V1-C

`diagram_labeling` est une question fermée structurée autour d'un diagramme sémantique natif : `layout`, `nodes`, `groups` optionnels, `edges`, puis `slots` ancrés sur un node ou une edge.

Payload interne : contient `diagram`, `slots`, `correctValues` et `explanation`.

Payload public pré-submit : contient `id`, `questionKind`, `prompt`, `instruction`, `difficulty`, `cognitiveSkill`, `sourceChunkIds`, `diagram` et `slots`, sans correction privée.

Answer submit : `{ questionId, questionKind: 'diagram_labeling', values: [{ slotId, optionId }] }`.

Correction post-submit : `{ correctValues: [{ slotId, optionId }] }` dans le payload de correction, avec `isCorrect`, `partialScore` et `explanation` au niveau item.

## 6. Genkit

- Schema Zod ajouté pour `diagram_labeling` : layouts allowlistés, nodes 2..8, groups <= 4, edges 0..12, slots 2..8, options 2..6.
- Prompt principal et prompt de réparation mis à jour.
- Le prompt interdit explicitement les réponses libres et les rendus HTML/SVG/Mermaid/Markdown/widget/renderPayload/image URL/canvas/code/markup.
- V1-021 et suivants restent interdits : `calculation_mcq`, `image_choice`, `fill_blank_dropdown`.
- Mix préservé : 6 = V1-A, 8 = V1-017, 10 = V1-018, 11 = V1-019, 12 = V1-020 full.
- Tests Genkit mockés uniquement, aucun provider réel.

## 7. Validation/scoring

Validation diagram :

- nodes : 2 à 8, IDs uniques.
- groups : 0 à 4, IDs uniques.
- edges : 0 à 12, IDs uniques, `fromNodeId`/`toNodeId` existants.
- node `groupId` optionnel mais forcément connu s'il est fourni.
- slots : 2 à 8, IDs uniques, anchorType `node` ou `edge`, anchorId existant.
- options par slot : 2 à 6, IDs uniques.
- `correctValues` complet, sans doublon, options existantes dans le slot.
- Submit complet, sans doublon, sans slot inconnu, sans option inconnue, sans réponse libre.

Scoring : full correct si toutes les valeurs correspondent exactement; sinon incorrect. Pas de partial score V1-020. La comparaison des couples `slotId/optionId` est structurée, pas concaténée, pour éviter les faux positifs avec des IDs contenant des séparateurs.

## 8. Flutter

Voir rapport app séparé. Côté API, aucune dépendance Flutter.

## 9. Anti-fuite

Pré-submit start/get ne contiennent jamais `correctValues`, `explanation`, `correction`, `score`, `feedback`, `modelAnswer`, ni aucun `correct*`. Le mapper public clone explicitement seulement les champs publics.

Les champs de rendu arbitraire sont rejetés par la validation contractuelle, par le quality gate public et par le DTO submit. Post-submit peut contenir correction, score et explication.
...
- Validator : fixture valide, nodes hors bornes, groups trop nombreux, edges trop nombreux/références inconnues, slots trop peu/trop nombreux, anchors invalides, options hors bornes/dupliquées, `correctValues` incomplets/inconnus.
- Public mapper : retrait de `correctValues` et `explanation`.
- Quality gate : anti-fuite pré-submit et rejet des clés arbitraires de rendu.
- Scorer : exact => correct, valeur fausse => incorrect, doublon/slot inconnu/option inconnue/incomplet => erreur, régression sur IDs avec séparateurs.
- Genkit : schema valide/invalide, prompt mentionne `diagram_labeling`, interdit widget libre, HTML/SVG/Mermaid et V1-021.
- E2E : start/get/result/submit diagram_labeling, invalides DTO et anti-fuite.

## 12. Validations lancées avec résultats

- `npm test -- rich-closed --runInBand` : passé, 8 suites / 163 tests.
- `npm test -- activities --runInBand` : passé, 17 suites passées, 1 skipped, 260 tests passés, 1 skipped.
- `npm run test:e2e -- --runInBand` : passé, 2 suites / 23 tests.
...
- Review Genkit sub-agent : `npm test -- --runInBand rich-closed-question-generation-profile.spec.ts genkit-rich-closed-question.generator.spec.ts` passé, 2 suites / 40 tests.

## 13. Validations non lancées avec justification

- Aucun provider IA réel lancé : les tests Genkit restent mockés conformément au prompt.
- Aucune migration Prisma lancée : aucun changement de schema nécessaire.
- Aucun test de V1-021 lancé : hors périmètre.

## 14. Risques restants

- Le diagramme est volontairement sémantique et textuel : pas de rendu graphique final dans ce lot.
- Scoring full-correct uniquement; pas de score partiel par slot.
- Le mix automatique n'inclut V1-020 qu'à 12 questions; pour des exercices plus courts, `diagram_labeling` reste à demander explicitement ou via ce count.
...
- Backend contract : sub-agent dédié, aucun problème bloquant.
- Backend Genkit/schema : sub-agent dédié, aucun problème bloquant, tests ciblés passés.
- Backend scoring/validation : sub-agent dédié, un risque de comparaison concaténée trouvé et corrigé.
- Public mapper anti-fuite : sub-agent dédié, renforcement du quality gate public sur les clés arbitraires de rendu.
- Flutter parser/model : sub-agent dédié, un durcissement de correction post-submit côté UI identifié et traité dans l'app.
- Flutter widget : sub-agent dédié, aucun problème bloquant.
- Tests/docs/sécurité : passe finale manuelle, validations complètes passées, aucun secret/provider réel/widget libre/V1-021 détecté.

## 17. Critique honnête du prompt initial

Le prompt est cohérent et utilement strict. La seule tension pratique est l'exigence de contenu complet du rapport lui-même : l'inclure dans sa propre section 18 créerait une récursion infinie. Le rapport est donc listé comme fichier créé, mais son propre contenu n'est pas recopié dans sa section 18; tous les autres fichiers touchés sont inclus intégralement.
