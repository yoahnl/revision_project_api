...
Lot V1-018 réalisé côté API. Deux nouveaux types rich closed fermés sont supportés de bout en bout : `true_false_grid` et `cause_consequence`.

Le backend ajoute le contrat, la validation stricte, le mapper public anti-fuite, le parsing submit, le scoring exact, les corrections post-submit, le générateur Genkit mockable, une fixture V1-B full à 10 questions, les tests unitaires et le smoke E2E. V1-A et V1-017 restent compatibles.

V1-018 est validable côté API.
...
- Tests rich closed, activities, Genkit, validator, scorer, public mapper, E2E.
- Rapports et roadmap V1 précédents, notamment V1-015/V1-016 et V1-017.
...
- Derniers commits initiaux :
  - `8e8cb5a 017: Intégration du slider de date pour la timeline et corrections associées`
...
- Repos non modifiés : aucun dans ce lot global. Le repo Flutter `revision_app` est modifié séparément avec son propre rapport V1-018.
...
- Ajout des question kinds `true_false_grid` et `cause_consequence`.
- Ajout des types internes, publics, réponses submit et corrections post-submit.
- Validation stricte des contrats et des réponses utilisateur.
- Mapper public pré-submit explicite sans correction privée.
- Scoring backend exact, sans score partiel pour ces deux types.
- Submit controller étendu avec validation DTO stricte.
- Genkit schema/prompt/profile/tests mis à jour sans provider réel.
- Fixture V1-B full dédiée à 10 questions.
- Smoke E2E V1-018 start/get/submit/result/invalid/anti-fuite.
- Roadmap API V1 mise à jour.

### Frontend app

Le frontend est traité dans le rapport app `revision_app/docs/v1/ROADMAP_EXECUTION_LOT_V1_018_TRUE_FALSE_GRID_CAUSE_CONSEQUENCE.md`.

## 5. Contrat V1-B

### `true_false_grid`

Payload interne : question fermée avec `rows: Array<{ id, statement, context? }>`, `correctValues: Array<{ rowId, value: boolean }>` et `explanation`.

Payload public pré-submit : `id`, `questionKind`, `prompt`, métadonnées, `instruction?`, `rows`. Les champs `correctValues` et `explanation` sont privés.

Answer shape : `{ questionId, questionKind: 'true_false_grid', values: Array<{ rowId, value }> }`.

Correction shape : statut backend + `correctValues` + `explanation` dans le résultat post-submit.

### `cause_consequence`

Payload interne : question fermée avec `causes`, `consequences`, `correctPairs: Array<{ causeId, consequenceId }>` et `explanation`.

Payload public pré-submit : `id`, `questionKind`, `prompt`, métadonnées, `instruction?`, `causes`, `consequences`. Les champs `correctPairs` et `explanation` sont privés.

Answer shape : `{ questionId, questionKind: 'cause_consequence', pairs: Array<{ causeId, consequenceId }> }`.

Correction shape : statut backend + `correctPairs` + `explanation` dans le résultat post-submit.

## 6. Genkit

- Schémas Zod stricts ajoutés pour `true_false_grid` et `cause_consequence`.
- Prompt principal mis à jour pour déclarer tous les types V1-B comme fermés.
- Prompt de réparation aligné sur les nouveaux types et les interdictions.
- Interdiction explicite des réponses libres, HTML/SVG/Mermaid, widget libre, champs arbitraires et types V1-019+.
- Mix par défaut préservé : 6 questions = V1-A, 8 questions = V1-017, V1-018 seulement à partir de 10 questions par défaut ou via mix explicite validé.
- Aucun provider IA réel lancé en test.
...
- 3 à 8 lignes.
- IDs de lignes non vides et uniques.
- `correctValues` complet, strictement booléen, sans doublon ni ID inconnu.
- Réponse utilisateur complète, strictement booléenne, sans doublon ni ID inconnu.
- Scoring : correct uniquement si toutes les valeurs correspondent exactement.

### `cause_consequence`

- Au moins 3 causes et 3 conséquences.
- IDs non vides et uniques.
- `correctPairs` couvre chaque cause une fois, avec conséquence existante et non dupliquée.
- Validation corrigée pendant la review : les paires malformées ne sont plus filtrées silencieusement.
- Réponse utilisateur complète, sans doublon de cause, sans doublon de conséquence, sans ID inconnu.
- Scoring : correct uniquement si l'ensemble exact des paires correspond.

## 8. Flutter

Voir rapport app. Côté API, le résultat post-submit fournit les corrections nécessaires sans demander de calcul de score côté client.

## 9. Anti-fuite

- Le mapper public ne renvoie jamais `correctValues`, `correctPairs`, `explanation`, `correction`, `score`, `feedback`, `modelAnswer` ni aucun champ `correct*` pré-submit.
- Le controller rejette les answers submit qui transportent des champs de correction ou de feedback.
- Les tests mapper et E2E scannent les payloads imbriqués start/get avant submit.
- Post-submit peut exposer correction, score et explication.
...
- Validator : fixtures valides, bornes de grille, IDs dupliqués, correction incomplète/inconnue/malformée, paires non univoques.
- Public mapper : anti-fuite pour les 10 kinds, fixture V1-B full.
- Scorer : exact correct/incorrect, erreurs submit doublon/inconnu/incomplet/non-bool.
- Genkit : schema V1-018, prompt, allowlist, mix 6/8/9/10.
- E2E : start/get anti-fuite V1-018, submit/result, invalid submit DTO, invalid submit sémantique.
- Non-régression : V1-A + V1-017 restent couverts.

## 12. Validations lancées avec résultats

- `npm test -- rich-closed --runInBand` : passé, 8 suites, 126 tests.
- `npm test -- activities --runInBand` : passé, 17 suites passées, 1 skipped, 223 tests passés, 1 skipped.
- `npm run test:e2e -- --runInBand` : passé, 2 suites, 21 tests.
- `npm test -- revision --runInBand` : passé, 15 suites, 87 tests.
- `npm test -- revision-session --runInBand` : passé, 6 suites, 41 tests.
- `npm test -- revision-sessions --runInBand` : passé, 6 suites, 41 tests.
- `npm run lint:check` : passé.
- `npm run build` : passé.
- `npx prettier --write ...` sur les fichiers TS touchés : passé.
- `git diff --check` : passé.

## 13. Validations non lancées avec justification

- Aucun provider IA réel : volontairement non lancé, les tests Genkit restent mockés.
- Aucune migration Prisma : non nécessaire pour des payloads JSON existants.
- Aucune action Dokploy : hors périmètre et interdite.

## 14. Risques restants

- Le mix par défaut n'introduit V1-018 qu'à partir de 10 questions ; c'est volontaire pour préserver 6 et 8 questions, mais cela limite l'exposition spontanée des nouveaux types.
- L'UI Flutter est volontairement minimale, donc la qualité pédagogique visuelle dépendra de la refonte prévue.
- Le catalogue produit devra rester synchronisé avec les futurs lots V1-C.

## 15. Recommandation prochain lot

V1-019 — Institution matrix V1-C, si l'objectif est de poursuivre l'élargissement du moteur rich closed. Aucun bis bloquant n'est requis côté API.

## 16. Passes de review

- Backend contract : sous-agent Meitner, aucun blocage.
- Backend Genkit/schema : sous-agent Euclid, aucun blocage ; suggestion `questionCount = 9` transformée en test.
- Backend scoring/validation : sous-agent Descartes, bug P1 corrigé sur les `correctPairs` malformés.
- Public mapper anti-fuite : sous-agent Schrodinger, aucun blocage.
- Flutter parser/model : traité dans le rapport app ; bug de parser côté app corrigé.
- Flutter widgets : traité dans le rapport app ; aucun blocage.
- Tests : review manuelle agent principal, validations listées ci-dessus.
- Sécurité : review manuelle agent principal, pas de secret, pas de provider IA réel, pas de migration, pas de widget libre, pas de V1-019 implémenté.

## 17. Critique honnête du prompt initial

Le prompt était cohérent et suffisamment borné. La seule vraie tension rencontrée vient d'un document catalogue côté app qui mentionnait encore 3 à 6 lignes pour `true_false_grid`, alors que le contrat V1-018 implémenté côté API est 3 à 8 lignes ; la doc app a été alignée. La demande d'inclure le contenu complet de tous les fichiers créés inclut théoriquement le rapport lui-même, ce qui créerait une récursion infinie ; ce rapport documente explicitement cette limite.
