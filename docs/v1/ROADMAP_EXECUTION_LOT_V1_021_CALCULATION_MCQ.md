...
V1-021 est implémenté côté API et validable. Le type fermé `calculation_mcq` est ajouté sans Prisma, sans migration, sans provider IA réel, sans widget libre et sans traiter V1-022.

Le backend recalcule déterministiquement la valeur attendue pour les deux modes autorisés : `absolute_majority_threshold` et `largest_remainder_target_party_seats`. Le scoring dérive maintenant l'ID de correction depuis cette valeur recalculée, puis le payload public pré-submit reconstruit les données par allowlist explicite pour ne laisser passer aucun champ privé imbriqué.

## 2. Sources inspectées

- `src/modules/activities/application/rich-closed-questions/rich-closed-question.types.ts`
- validators, quality gates, public mapper, scorer, fixtures rich closed
...
- `prisma/demo-seed.ts` inspecté indirectement pour confirmer qu'aucune migration/seed destructif n'était requis
- rapports V1-019/V1-020 et `docs/v1/ROADMAP_EXECUTION_PLAN_V1.md`
...
- derniers commits initiaux : `07f6e00 020: Intégration de l'étiquetage de diagrammes`, `4f51fcd 019...`, `4092741 018...`
- repo app touché séparément : voir `revision_app/docs/v1/ROADMAP_EXECUTION_LOT_V1_021_CALCULATION_MCQ.md`
- aucun commit, merge, rebase, tag ou push lancé

## 4. Périmètre réalisé

Backend API :
- ajout du discriminant `calculation_mcq` ;
- contrat interne/public ;
- calculateur déterministe ;
- validation stricte des deux modes ;
- mapper public anti-fuite par allowlist, y compris champs imbriqués ;
- parsing submit ;
- scoring full-correct dérivé de la valeur recalculée ;
- correction post-submit ;
- Genkit schema/prompt/mix ;
- fixture V1-C calculation à 13 questions ;
- tests unitaires, Genkit mocké et E2E.

Frontend app : voir le rapport app séparé.

## 5. Contrat V1-C

`calculation_mcq` contient :
- `scenario` ;
- `calculation` discriminé par `mode` ;
- `choices: Array<{ id, label, value }>` ;
- `correctChoiceId` privé ;
- `explanation` privée pré-submit.

Payload public : `id`, `questionKind`, `prompt`, metadata pédagogique, `instruction`, `scenario`, `calculation`, `choices`.

Answer submit : `{ questionId, questionKind: 'calculation_mcq', choiceId }`.

Correction post-submit : `{ correctChoiceId, expectedValue, workedSteps }`, où `correctChoiceId` est dérivé de l'unique choice dont `value === expectedValue` recalculé.

## 6. Genkit

- schema Zod ajouté pour `calculation_mcq` et les deux modes autorisés ;
- prompt principal et prompt de réparation mentionnent explicitement le type fermé ;
- prompt interdit réponses libres, formule libre, HTML/SVG/Mermaid/widget libre, code/rendu arbitraire, D'Hondt, Sainte-Laguë, seuils et votes blancs/nuls ;
- mix V1-A à 6, V1-017 à 8, V1-018 à 10, V1-019 à 11, V1-020 à 12 préservés ;
- `calculation_mcq` entre dans le mix automatique à 13 questions ;
- tests Genkit mockés uniquement, aucun provider réel.

## 7. Validation/scoring

`absolute_majority_threshold` :
- `validVotes` entier strictement positif et borné ;
- valeur attendue recalculée : `Math.floor(validVotes / 2) + 1`.

`largest_remainder_target_party_seats` :
- 2 à 8 partis ;
- sièges et votes entiers bornés, avec votes de parti `>= 0` ;
- target party existante ;
- quota de Hare par restes entiers ;
- rejet des égalités de reste qui franchissent la frontière du dernier siège attribué.

Scoring : full correct si `choiceId` vaut l'ID dérivé de `expectedValue` recalculé. Le backend ne fait pas confiance à `correctChoiceId` pour décider la correction ; il l'utilise comme donnée privée validée au contrat, mais le scorer dérive la source de vérité.

## 8. Flutter

Voir rapport app. Côté API, le contrat public a été borné pour que Flutter n'ait jamais à rendre du JSON arbitraire ni à recalculer le score.

## 9. Anti-fuite

Pré-submit start/get ne contiennent pas : `correctChoiceId`, `expectedValue`, `workedSteps`, `explanation`, `correction`, `score`, `feedback`, `modelAnswer`, `formula`, `expression`, `rawFormula`, `calculationCode`, `javascript`, `python`, `eval`, `Function`, `render`, ni champ `correct*`.

Post-submit peut contenir correction, score et explication.

## 10. Fichiers créés/modifiés/supprimés

Créés :
- `src/modules/activities/application/rich-closed-questions/rich-closed-question-calculation.ts`
- `src/modules/activities/application/rich-closed-questions/rich-closed-question-calculation.spec.ts`
...
- calculateur déterministe ;
- validators `calculation_mcq` ;
- public mapper anti-fuite, y compris champs privés imbriqués ;
- quality gate anti-formule/rendu ;
- scorer submit/correction et non-confiance envers `correctChoiceId` ;
- Genkit schema/prompt/mix ;
- fixture V1-C calculation ;
- E2E start/get/submit/result avec anti-fuite.

## 12. Validations lancées avec résultats

- `npm test -- rich-closed --runInBand` : PASS, 9 suites, 199 tests.
- `npm test -- activities --runInBand` : PASS, 18 suites passées, 1 skipped, 296 tests passés, 1 skipped.
- `npm run test:e2e -- --runInBand` : PASS, 2 suites, 24 tests.
- `npm test -- revision --runInBand` : PASS, 15 suites, 87 tests.
- `npm test -- revision-session --runInBand` : PASS, 6 suites, 41 tests.
- `npm test -- revision-sessions --runInBand` : PASS, 6 suites, 41 tests.
- `npm run lint:check` : PASS après correction du test anti-fuite imbriqué.
- `npm run build` : PASS.
- `git diff --check` : PASS après génération finale des rapports.

## 13. Validations non lancées avec justification

Aucune validation backend demandée n'a été volontairement omise. Aucun provider IA réel, aucune migration et aucune action Dokploy n'ont été lancés.

## 14. Risques restants

- Le mode plus fort reste rejette seulement les égalités qui changent l'attribution du dernier siège ; c'est volontaire mais à réauditer avant extension électorale.
- Le mix automatique ajoute `calculation_mcq` à 13 questions ; les prompts/tests protègent 6/8/10/11/12.
- Les `workedSteps` sont explicatifs, pas une API de calcul formelle.

## 15. Recommandation prochain lot

Recommandation : V1-022 — Image choice/personnages historiques V1-D, si le besoin est confirmé. Aucun bis backend n'est nécessaire pour V1-021.

## 16. Passes de review

- backend contract : P2 mapper public trop permissif corrigé par allowlist explicite.
- backend Genkit/schema : OK, schema strict et prompt anti-formule/widget libre.
- backend scoring : P1 corrigé, le scorer dérive l'ID correct depuis `expectedValue` recalculé.
- public mapper anti-fuite : OK après test de champs privés imbriqués.
- Flutter parser/model : P2 app traité dans le rapport app.
- Flutter widget : voir rapport app.
- correction UI : voir rapport app.
- tests/docs/sécurité : mismatch votes zéro traité côté app ; pas de secret, provider réel, migration ni Dokploy.

## 17. Critique honnête du prompt initial

Prompt cohérent et utilement strict. Deux tensions notables : la section “contenu complet” crée une récursion si le rapport s'auto-inclut, donc le rapport est listé mais non auto-inclus ; et le lot demande beaucoup de validations longues, ce qui impose de ne pas paralléliser les commandes Flutter côté app à cause du verrou de startup.

## 18. Notes anti-récursion

La section suivante contient tous les fichiers touchés du repo API sauf le présent rapport lui-même.
